        ...
        <div class="container mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
            ...
            <div class="flex items-center space-x-1 text-xs md:text-sm">
                <?php if (isset($_SESSION['username'])) : ?>
                    <a href="main/dashboard.php" class="text-white bg-[#1DA1F2] border-none rounded px-2 md:px-4 py-2 m-1 md:m-2 transition-colors duration-300 hover:bg-[#0d8ae6]"><i class="fa-solid fa-clipboard"></i> ‎ Dashboard</a>
    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 my-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 flex-grow">
        <?php while ($row = $result->fetch_assoc()) : ?>
            <div class="bg-white p-4 border border-gray-300 rounded shadow-sm">
                ...
                <p class="max-w-full overflow-hidden text-ellipsis whitespace-nowrap text-sm text-black mb-1"><?= htmlspecialchars($row['description']); ?></p>
                <a class="text-sm text-white bg-[#1DA1F2] px-4 py-2 rounded transition-colors duration-300 hover:bg-[#0d8ae6] mt-1 mb-2 inline-block" href="main/news_detail.php?id=<?php echo $row['id']; ?>"><i class="fa-solid fa-book-open"></i> ‎ Read more</a>
            </div>
        <?php endwhile; ?>
    </div>
